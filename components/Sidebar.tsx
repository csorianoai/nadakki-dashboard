...
const MENU_ITEMS = [
  ...
  { name: "Analytics", href: "/analytics", icon: BarChart3 },
  {
    ...
    icon: Bot,
    children: [
      { name: "Legal", href: "/legal", icon: Scale, badge: "33" },
      { name: "Logistica", href: "/logistica", icon: Truck, badge: "24" },
      { name: "Contabilidad", href: "/contabilidad", icon: Calculator, badge: "23" },
      { name: "Presupuesto", href: "/presupuesto", icon: PieChart, badge: "14" },
      { name: "Originacion", href: "/originacion", icon: Banknote, badge: "11" },
      { name: "RRHH", href: "/rrhh", icon: Users, badge: "11" },
      { name: "Educacion", href: "/educacion", icon: GraduationCap, badge: "10" },
      { name: "Investigacion", href: "/investigacion", icon: Search, badge: "10" },
      { name: "VentasCRM", href: "/ventascrm", icon: ShoppingCart, badge: "10" },
      { name: "RegTech", href: "/regtech", icon: BookOpen, badge: "9" },
      { name: "Compliance", href: "/compliance", icon: Shield, badge: "6" },
      { name: "Decision", href: "/decision", icon: Brain, badge: "6" },
      { name: "Experiencia", href: "/experiencia", icon: Heart, badge: "6" },
      { name: "Fortaleza", href: "/fortaleza", icon: Shield, badge: "6" },
      { name: "Inteligencia", href: "/inteligencia", icon: Zap, badge: "6" },
      { name: "Operacional", href: "/operacional", icon: Cog, badge: "6" },
      { name: "Orchestration", href: "/orchestration", icon: Server, badge: "6" },
      { name: "Recuperacion", href: "/recuperacion", icon: RefreshCw, badge: "6" },
      { name: "Vigilancia", href: "/vigilancia", icon: Eye, badge: "6" },
    ]
  },
  {
    name: "Admin",
        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-white">NADAKKI AI</h1>
                <p className="text-xs text-gray-500">Enterprise Suite v4.0</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        <button onClick={() => setCollapsed(!collapsed)} className="p-2 hover:bg-white/5 rounded-lg transition-colors">
            ...
            {item.children ? (
              <>
                <button onClick={() => toggleMenu(item.name)}
                  className={"w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all " + (expandedMenus.includes(item.name) ? "bg-white/5 text-white" : "text-gray-400 hover:bg-white/5 hover:text-white")}>
                ...
                <div className={"flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all " + (isActive(item.href) ? "bg-gradient-to-r from-purple-500/20 to-cyan-500/20 text-white border border-purple-500/30" : "text-gray-400 hover:bg-white/5 hover:text-white")}>
                  ...
                  {!collapsed && (
                    <>
                      ...
                      {item.badge && <span className="px-2 py-0.5 text-xs rounded-full bg-purple-500/20 text-purple-400">{item.badge}</span>}
                    </>
                  )}
                </div>
              </Link>
      ...
      {!collapsed && (
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-purple-500/20">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center text-white text-sm font-bold">C</div>
