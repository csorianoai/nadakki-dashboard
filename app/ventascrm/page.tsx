...
export default function ventascrmPage() {
  ...
  useEffect(() => {
    fetch("${process.env.NEXT_PUBLIC_API_BASE_URL}/api/catalog/ventascrm/agents")
      .then((res) => res.json())
  ...
  const executeAgent = async (agentId: string) => {
    ...
    setShowModal(true);
    try {
      const response = await fetch("${process.env.NEXT_PUBLIC_API_BASE_URL}/agents/ventascrm/" + agentId + "/execute", {
        method: "POST",
  ...
  return (
    <div style={{ padding: 40, backgroundColor: "#0a0f1c", minHeight: "100vh" }}>
      <h1 style={{ fontSize: 32, fontWeight: 800, color: "#f8fafc", marginBottom: 8 }}>ðŸ¤ Ventas CRM Core</h1>
