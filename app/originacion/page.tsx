...
import { Banknote, Bot, CheckCircle, Clock, Search, ArrowRight, RefreshCw } from "lucide-react";
...
export default function OriginacionPage() {
  ...
  const fetchAgents = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://nadakki-ai-suite.onrender.com/api/catalog/originacion/agents");
  ...
  return (
    <div className="ndk-page ndk-fade-in">
      <NavigationBar backHref="/">
        ...
        <StatusBadge status="active" label="Originacion" size="lg" />
      </NavigationBar>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-bold bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent">Originacion</h1>
            <p className="text-gray-400 mt-1">{agents.length} agentes de IA</p>
          </div>
        </div>
      </motion.div>
      <div className="grid grid-cols-4 gap-6 mb-8">
        <StatCard value={agents.length.toString()} label="Agentes" icon={<Bot className="w-6 h-6 text-green-400" />} color="#22c55e" />
        <StatCard value="10" label="Workflows" icon={<Banknote className="w-6 h-6 text-emerald-400" />} color="#10b981" />
        <StatCard value="99%" label="Precision" icon={<CheckCircle className="w-6 h-6 text-cyan-400" />} color="#06b6d4" />
        ...
        <div className="grid grid-cols-3 gap-4">
          {filteredAgents.map((agent, i) => (
            <motion.div key={agent.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}>
              <Link href={'/originacion/' + agent.id}>
                <GlassCard className="p-5 cursor-pointer group h-full hover:bg-white/10 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-white truncate group-hover:text-green-400 transition-colors">{agent.name}</h3>
                      <p className="text-xs text-gray-400">{agent.category}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-green-400 transition-all" />
                  </div>
                </GlassCard>
              </Link>
            </motion.div>
