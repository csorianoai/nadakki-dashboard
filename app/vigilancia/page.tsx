...
export default function vigilanciaPage() {
  ...
  useEffect(() => {
    fetch("${process.env.NEXT_PUBLIC_API_BASE_URL}/api/catalog/vigilancia/agents")
      .then((res) => res.json())
  ...
  const executeAgent = async (agentId: string) => {
    ...
    setShowModal(true);
    try {
      const response = await fetch("${process.env.NEXT_PUBLIC_API_BASE_URL}/agents/vigilancia/" + agentId + "/execute", {
        method: "POST",
  ...
  return (
    <div style={{ padding: 40, backgroundColor: "#0a0f1c", minHeight: "100vh" }}>
      <h1 style={{ fontSize: 32, fontWeight: 800, color: "#f8fafc", marginBottom: 8 }}>ðŸ‘ï¸ Vigilancia Core</h1>
        ...
        {agents?.map((agent) => (
          <div key={agent.id} style={{ backgroundColor: "rgba(30,41,59,0.5)", border: "1px solid rgba(51,65,85,0.5)", borderRadius: 12, padding: 20 }}>
            ...
            <button onClick={() => executeAgent(agent.id)} disabled={executing === agent.id} style={{
              ...
              backgroundColor: executing === agent.id ? "#6b7280" : "#EF4444",
