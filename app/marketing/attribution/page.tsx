...
export default function MarketingattributionPage() {
    ...
    fetch("https://nadakki-ai-suite.onrender.com/api/catalog/marketing/agents")
      ...
      .then((data) => {
        const filtered = (data.agents || []).filter((a: Agent) => 
          a.name?.toLowerCase().includes("attribut") || 
          a.category?.toLowerCase().includes("attribut")
        );
  ...
  if (loading) return <div style={{ padding: 40, backgroundColor: "#0a0f1c", minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center" }}><div style={{ color: "#94a3b8" }}>Cargando agentes de Attribution...</div></div>;

  return (
    <div style={{ padding: 40, backgroundColor: "#0a0f1c", minHeight: "100vh" }}>
      <h1 style={{ fontSize: 32, fontWeight: 800, color: "#f8fafc", marginBottom: 8 }}>Attribution</h1>
