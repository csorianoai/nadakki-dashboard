        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-sm text-gray-400 mb-2">Seleccionar Core</label>
                <select
                  value={selectedCore}
                ...
                >
                  ...
                  {Object.values(CORES_CONFIG).map((core) => (
                    <option key={core.id} value={core.id}>{core.displayName}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-2">Seleccionar Agente</label>
                <select
                  value={selectedAgent}
                  onChange={(e) => setSelectedAgent(e.target.value)}
                  disabled={!agents.length}
                  className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white disabled:opacity-50"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full p-3 rounded-xl bg-white/5 border border-white/10 text-white font-mono text-sm"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl font-bold text-black bg-gradient-to-r from-cyan-400 to-purple-500 hover:scale-105 transition-transform disabled:opacity-50"
              >
            ...
            <div>
              ...
              <div className="h-96 p-4 rounded-xl bg-white/5 border border-white/10 overflow-auto">
                {result ? (
